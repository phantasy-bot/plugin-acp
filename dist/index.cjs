...
__export(index_exports, {
  ...
  ACPMarketplaceClient: () => ACPMarketplaceClient,
  ACPOfferingRegistry: () => ACPOfferingRegistry,
  ACPPlugin: () => ACPPlugin,
  ACPSellerRuntime: () => ACPSellerRuntime,
  ACPService: () => ACPService,
  AGDPTracker: () => AGDPTracker,
  ClawHubPublisher: () => ClawHubPublisher,
  createACPOfferingRegistry: () => createACPOfferingRegistry,
  createACPSellerRuntime: () => createACPSellerRuntime,
  createACPService: () => createACPService,
  createAGDPTracker: () => createAGDPTracker,
  createOfferingFromSkill: () => createOfferingFromSkill,
  default: () => acp_plugin_default,
  getACPEscrow: () => getACPEscrow,
  getACPEvaluator: () => getACPEvaluator,
  getACPMarketplaceClient: () => getACPMarketplaceClient,
  getACPOfferingRegistry: () => getACPOfferingRegistry,
  getACPSellerRuntime: () => getACPSellerRuntime,
  getACPService: () => getACPService,
  getAGDPTracker: () => getAGDPTracker,
  getClawHubPublisher: () => getClawHubPublisher,
  getRegisteredSkillOfferings: () => getRegisteredSkillOfferings,
  registerSkillAsOffering: () => registerSkillAsOffering,
  skillToOfferingHandlers: () => skillToOfferingHandlers,
  syncAllSkillsToOfferings: () => syncAllSkillsToOfferings,
  unregisterSkillOffering: () => unregisterSkillOffering
});
module.exports = __toCommonJS(index_exports);

// src/acp-plugin.ts
var import_plugins = require("@phantasy/agent/plugins");
var import_plugin_runtime2 = require("@phantasy/agent/plugin-runtime");

// src/runtime/shared.ts
var import_plugin_runtime = require("@phantasy/agent/plugin-runtime");
function createACPModuleLogger(name) {
  return (0, import_plugin_runtime.createPluginModuleLogger)(name);
}
...
var ACPService = class {
  ...
  async request(endpoint, options = {}) {
    ...
    const response = await (0, import_plugin_runtime.fetchWithTimeout)(url, {
      timeout: 3e4,
      ...options,
      headers: {
...
var ClawHubPublisher = class {
  async publishSkill(skillName, config) {
    try {
      const skillLoader = (0, import_plugin_runtime.getSkillLoader)();
...
// src/acp-plugin.ts
var log5 = (0, import_plugin_runtime2.createPluginModuleLogger)("ACPPlugin");
...
function getAgentId(url) {
  return url.searchParams.get("agentId") || (0, import_plugin_runtime2.getActiveAgentId)();
}
...
function getAvailableSkills() {
  return (0, import_plugin_runtime2.getSkillLoader)().getAllSkills().map((skill) => ({
...
var ACPPlugin = class extends import_plugins.BasePlugin {
  name = "acp";
  version = "0.1.0-beta";
...
var ACPSellerRuntime = class {
  ...
  start() {
    ...
    this.wss = new import_ws.default.Server({
      server: this.server,
      path: "/acp/sell"
    });
    this.wss.on("connection", (socket) => {
      const clientId = (0, import_plugin_runtime.createRuntimeId)("client");
